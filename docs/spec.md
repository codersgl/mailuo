...
- 父任务自己的 `duration_minutes` 不是展示口径（字段保留）：卡片不画工期胶囊，改为画**子树聚合**——Σ 未归档叶子的已用 / Σ 未归档叶子的工期。只要有一片未归档叶子未估工期，分母就算「未估」，只报已用，不给进度条与临近 / 超期。两个和都是工作量口径：叶子可以并行，Σ工期 不是日历工期（见 `docs/decisions.md` D77）。抽屉里的工期输入对有子任务的任务暂时没有出口。
