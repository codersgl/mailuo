...
- 父任务自己的 `duration_minutes` 不再是展示口径（字段保留）：卡片不画工期胶囊，卡片与任务树都不画工期提醒，抽屉里的工期输入是唯一出口。要表达「这个分支投入了多少」得用子树聚合，本版未做（见 `docs/decisions.md` D76）。
...
- 不提醒的情形：未估工期（`NULL`）与工期 0（瞬时任务）没有可用的时间窗；完成列与已归档任务不再提醒；有子任务的父任务不提醒（见上一条）。前三类仍然照常记账（`spent_minutes` 照样累加），只是不产生标记；父任务是从长出第一个子任务的那一刻起就不再有已用增长。
