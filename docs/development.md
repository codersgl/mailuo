...
七份文档各管一段，同一件事只在一处写全，改动时先改权威处再同步其余：

| 文件 | 管什么 | 谁可以改 |
| --- | --- | --- |
| `README.md` | 使用者视角：功能、安装、命令行参数、数据位置、常见问题 | 任何人 |
| `docs/intend.md` | 需求与待办（Features / Tasks / Issues / Docs） | 只由用户改 |
| `docs/spec.md` | 规范：数据模型、API 契约、状态语义、界面行为 | 只由用户改 |
| `docs/decisions.md` | 决策记录：每条为什么这么定、代价与边界，只追加 | 任何人（只追加） |
| `docs/audit-2026-09-23.md` | 2026-09-23 的代码质量与安全审查快照，含未处理条目 | 任何人（只追加批注） |
| `docs/development.md` | 本文：开发命令、CI、发布、生产运行、环境变量、图标 | 任何人 |
| `AGENTS.md` | 给 AI 助手的规则 | 只由用户改 |

几处**权威源**（其它地方与它冲突时以它为准）：

- CLI 的参数、优先级与 `--help` 文案：`bin/mailuo.mjs`（README 的参数表是它的使用者视角摘要）。
- 界面与接口契约：`docs/spec.md`。
- 服务端行为细节（默认端口、信任边界、数据库路径）：`apps/api/src/config.ts`，逐项写在本文「环境变量」一节。
- 技术栈与目录结构在本文与 `docs/spec.md` 各写了一份：`docs/spec.md` 是权威，本文那份是开发者视角的摘要，改了一处要顺手看另一处。

两条**引用约定**，用来避免文档互相指错：

- `docs/decisions.md` 与 `docs/audit-2026-09-23.md` 里的行号与文件名是**写作当时的快照**，不是活引用。查代码请按里面的符号名（函数名、常量名、测试名）找，不要按行号跳。
- 决策记录只追加。后一步推翻了前面某条时，在前一条就地补一句「（Dxx 已实现 / 已作废）」，不要改写原来的理由。
