...
记录影响后续实现的决策，只追加：后一步推翻前面某条时，在前一条就地补一句「（Dxx 已实现 / 已作废）」，
不改写原来的理由。需求与规范见 `docs/intend.md`、`docs/spec.md`，两者由用户维护；`AGENTS.md` 同样只由用户改。
各文件分工与权威源见 `docs/development.md` 的「文档」一节。

正文里的行号与文件名是写作当时的快照，查代码请按符号名（函数名、常量名、测试名）找。

## 索引

按日期分三段，段内按编号列出标题；查找用 `D<编号>`。新增条目时在对应段末补一行。
...
- D76 第 38 步：父任务的状态由子任务推导（2026-09-25，分支 feat/derived-status，用户授权改规范）
- D77 第 39 步：父任务的分支投入用子树聚合表达（2026-09-25，分支 feat/subtree-time，用户授权改规范）
...
- 生产托管的 SPA fallback（见上）。（D56 已实现，这条不再成立。）
...
注：验收用的是 dev server（真实 API + 真实库）。`docs/spec.md` 里「生产由 Hono `serveStatic` 托管 `apps/web/dist`」这件事代码里还没实现，所以没有用「构建产物 + 真实 API 同源」这一种跑法。（D56 已实现，这条不再成立。）
...
### 审计 D5：`updateTaskFields` 改用 `immediate` 事务
...
### 审计 D6 + D10：前置校验抽一处，三处不可达的二次 404 收敛成断言
...
需求（`docs/intend.md` 的 Docs）：「整理文档，提高文档的质量和可维护性」。用户同一步还明确：
`AGENTS.md` 与 `docs/intend.md`、`docs/spec.md` 一样，只由用户改。

动手前先做了一次只读体检（子代理，逐条 grep + 读源码核对），拿到的是五类具体缺陷，而不是「感觉乱」：

1. **README 有已经过时的句子**：`包发布到 npm 后，可以不安装直接运行`——0.1.0 早在 D69 就发布了，
   这句会让读者以为现在还不能 `npx`。另一处「命令行启动不读仓库根的 `.env`」漏了 `HOST_ALLOW`：
   `bin/mailuo.mjs` 只在它非空时写进程环境，所以服务端读 `.env` 时仍会被填上，与
   `docs/development.md` 的环境变量一节矛盾。
2. **决策记录里三处对同一件事说法不同**：生产静态托管「还没实现」（第 322、790 行）与
   「D56 已实现」（第 282 行）。D56 确实实现了，「只追加」被误当成「不能补批注」。
3. **审查报告没有状态**：352 行、52 条结论里有 30 条已在 D55–D70 落地，正文却仍是「建议改代码」的
   语气；132 处行号引用全部冻结在基线 `b2722de`。另有一条交叉引用是错的——「折叠徽标不一致见
   D13」，而 D13 是「归档任务下禁止新建子任务」，C6 的修复记在 D58。
4. **决策记录没有索引**：2677 行、71 条，只能 grep；D59 内部还用了 `### D4：`/`### D5：`/
   `### D6 + D10：`/`### D11：` 当小标题，和真正的 D4/D5/D6/D10/D11 撞号。
5. **同一事实写两到三遍且没写「以谁为准」**：技术栈与目录结构（`development.md` vs `spec.md`）、
   CLI 参数（README vs `spec.md` vs `--help`）、「1 天 = 480 分钟 / 上限 9999 天」这类常量
   （`spec.md` 三处 + 代码四处）。
...
- README：删掉过时前提；补 `HOST_ALLOW` 例外并指向 `docs/development.md`；开发一节补审查报告链接。
- `docs/decisions.md`：322、790 两处补「（D56 已实现，这条不再成立。）」，与 282 的写法一致。
- `docs/audit-2026-09-23.md`：`D13` 改成 `D58`。
- `docs/development.md`：新增「文档」一节——七份文档各管什么、谁能改（`intend.md`/`spec.md`/
  `AGENTS.md` 只由用户改）、四处权威源（CLI 看 `bin/mailuo.mjs`、契约看 `spec.md`、服务端默认值看
  `apps/api/src/config.ts`、技术栈与目录以 `spec.md` 为准），以及两条引用约定：决策与审查报告里的
  行号是**快照**，查代码按符号名；决策只追加，但推翻旧条目时要**就地补一句作废批注**。
- `.github/workflows/ci.yml`：job 名从 `lint / typecheck / test / build` 改成实际顺序。

**可导航性与状态**（本步第二条提交）

- `docs/decisions.md` 头部加 D1–D73 索引：按 2026-09-22 / 09-23 / 09-24 三段，段内按编号列标题。
  索引由脚本一次性生成（从 `## D` 标题里取日期分组），之后手工维护，规则写在索引上方。
- D59 的四个小标题改成「审计 D4 / 审计 D5 / 审计 D6 + D10 / 审计 D11」。
- `docs/audit-2026-09-23.md` 加「修复状态（2026-09-24 补记）」：已落地条目按决策编号归拢
  （A1–A3→D55、B1→D56、B3/B8/B11/C1/C2/C4/C6→D58、B12/D4/D5/D6/D10/D11→D59、D3/E1–E3→D60、
  D1→D61、B2/B4–B7/B9/B10/D2→D62、D7 的门禁部分→D70），仍未处理的 17 项逐条写现状。C5 要写准：
  D54 已记录用户拍板「清掉搜索词，回到刚点的那一层看板」，并指派给 F 第 3 步，但 D58 的七条里
  没有它，代码也至今没实现——所以它是**已拍板未实现**，不是待拍板。报告头部写明正文的行号与计数
  停在 `b2722de`。

### 三个取舍（都是「可选复杂性」，写下来备查）

- **132 处行号不改成符号引用**：报告与决策记录是快照，行号是当时的坐标。把它们改写成当前的符号名
  会让一份历史记录看起来永远最新，反而更难判断哪句还成立。所以只在两处声明「行号是快照、按符号名
  查」，不改正文。代价：想顺着引用读代码的人要多做一次搜索。
- **索引不引入生成脚本**：72 行索引由脚本一次生成、之后手工维护。彻底的做法是
  `scripts/docs-index.mjs` + 一条断言索引与标题同步的用例（仓里已有 `bin/package.test.mjs` 这种
  「用例钉住元数据」的先例），但那要改 `package.json` 与新增用例，属于代码改动，单独一步做。
- **重复内容不删，先定权威**：技术栈/目录/CLI 参数被写了两三遍，直接删会让每份文档的读者都要跳出去
  看。这一步只写清谁权威，删并留给真出现漂移时再做。

### 验证

- 索引与标题一致：`grep -c '^## D'` 与索引里的条目数都是 72，三段范围与标题里的日期一致。
- 报告里各处行号引用未逐个复核（按上面的取舍，它们不被当成活引用），但「已落地」映射逐条对着
  决策记录正文复核过：D55 写的是 A1/A2/A3，D56 写的是 B1，D58 写的是 C1/C2/C4/C6/B3/B8/B11，
  D59 写的是 D4/D5/D6/D10/D11/B12，D60 写的是 D3/E1/E2/E3，D61 写的是 D1，D62 写的是
  B2/B4/B5/B6/B7/B9/B10/D2，D70 写的是审计 D7 的门禁部分。
- 「仍未处理」抽验：A4（`grep -rn 'chmod\|0o600' apps/api/src` 无命中）、C7（`App.tsx` 的
  `boardRef.current`/`pointerActiveRef.current` 与 `usePointerDrag` 的 `callbacks.current` 仍是
  渲染期赋值）、D12（`tsconfig.base.json` 仍缺那四个开关）、D13（`@types/node@^26` 对
  `engines.node >=22`）、E6（`deps.ts` 的 `[...new Set(...)]` 还在）、E8（`App.test.tsx` 的
  `query.includes('includeArchived')` 还在）。
- 相对链接逐个 `existsSync` 核对，改动后没有指向不存在文件的引用。
- 本步不跑测试：改动只有 markdown 与一个 workflow 的 job 名（只影响 Actions 里的显示名）。

### 审阅（子代理，只读）与更正

审阅结论：**有 1 条高、4 条中、若干低**，都集中在这一次新增的文字里，实现面的改动（README 的两处、
322/790 的批注、索引、D59 小标题、已落地映射、纪律与 markdown 卫生）逐条核对成立。逐条处置：

1. **（高，已改）把 C5 写成了「开放的产品决定 / 待拍板」**。D54 正文里用户已经拍板「清掉搜索词，
   回到刚点的那一层看板」，并指派到修复顺序第 3 步；D58 的七条里没有它，代码也没实现。也就是说
   C5 是**已拍板未实现**，我原来的写法等于把一个已决事项重新抛回给用户。已改掉四处（审查报告
   状态表与小结、D73 的做法与「没做的」）。
2. **（中，已改）「60 条结论」应为 52 条**（报告自己的条目统计与 D54 都写 52；「352 行」与
   「132 处行号引用」是对的，审阅用正则独立复算过）。
3. **（中，已改）E5 那行写宽了**：`apps/api/test/config.test.ts` 在审查基线时就同时有
   `expect(config.port).toBe(DEFAULT_PORT)` 与 `expect(DEFAULT_PORT).toBe(3001)`，api 这一条并不是
   同义反复；仍成立的是 `useNow.test.tsx` 与 `graph.test.ts` 两条。已限定。
4. **（中，已改）缺陷 5 举的第三个例子不成立**：README 当时写着「命令行与开发模式用的不是同一个库」
   并解释了两种入口，所以那不是「没写清」的问题。已换成「1 天 = 480 分钟 / 上限 9999 天」
   （`spec.md` 三处 + 代码四处）。顺带：那两个默认库已在并行一步 D72 里统一，这个例子的上下文也消失了。
5. **（中，已改）「这一组基本就是 F 第 8 步」不成立**：表里有 E4/E7/E8 属于第 6 步（让测试重新
   可信），C5 是产品决定，集合关系是「第 8 步 ∪ {C5, E4, E7, E8}」。已按这个划分改写这一段的
   框架，并说明它们不在本步范围。
6. **（低，已改）**D73 正文里引用的两个提交哈希：第二个在 amend 后已经变了。改成按内容指代
   （第一条/第二条提交），不再写哈希。
7. **（低，已改）**末尾「（待填）」占位：就是这一节。
8. **（低，已改）**「`DragGhost` 零引用」字面不成立（`useCardDrag.test.tsx` 里有一句注释提到它），
   改成「测试里不渲染 `DragGhost`」。
9. **（低，已改）**E7 应写成「只做了一半」：D60 已按报告建议改了用例名与注释（说明守不住什么），
   断言本身仍不能区分被测实现。
10. **（低，已改）**索引行数口径（71/72）与 D60「三组/四组」的措辞已对齐各自正文。
11. **（低，已改）**README 把 `HOST_ALLOW` 说成「命令行没设它时留空」不准确——它根本没有命令行
    开关。已改成「没有命令行开关，服务端启动读 `.env` 时生效」。审阅另实测了一个边界：shell 里
    显式设成空串时 `.env` 的值不会被补上（`process.loadEnvFile` 不覆盖已存在的变量），README 不
    展开这一层。
12. **（低，已改）**E9–E13 那行里「104 处 `toBeTruthy()`（现已 172 处）」是两个口径混用：报告原文
    的 104 是 `expect(getBy*())` 家族，今天同一口径是 169 处，172 是所有 `toBeTruthy` 出现数。
    已按一个口径写。

审阅自己声明**未能核对**的：索引生成脚本（本步明确不引入，没有生成记录可查）、「30 条已落地」
（按映射是 30/52，只能算近似）、以及没跑的测试与构建（纯文档步骤，且 worktree 没有 `node_modules`）。

### 没做的（可选复杂性）

- 不把 132 处行号改成符号引用；不做索引生成脚本与同步用例；不删重复内容（三条理由见上）。
- 不动 `docs/intend.md`、`docs/spec.md`、`AGENTS.md`：只由用户改。
- 不处理审查报告里未完成的加固项：C5 已由用户拍板但落在 D58 那一步的范围里，本步只如实记录；
  其余按报告自己的判断「等真遇到问题再动」。
- 不给文档加 markdown lint（例如 markdownlint）：本步只是先把事实改对，门禁是另一件事。
...
需求（`docs/intend.md` 的 Features 最后一条）：子任务有一个在进行中，所有祖先任务自动放置在进行中；
所有子任务完成，其父任务自动完成，除非没有子任务。

### 麻烦在哪：规范原本写的是反的

`docs/spec.md` 的「状态语义」里原本有一条「父任务的完成不自动推导。不实现『子任务全部完成则父任务
自动移入完成列』」。它与需求直接冲突，而规范只由用户改，所以先把冲突摆出来请用户拍板；用户授权
改规范并实现。本步动的就是那一节、外加「工期提醒」的计时口径、「归档」与「界面行为-拖拽」各一句，
以及两条写接口的契约描述。

### 决策一：完全推导，不是「只加两条单向规则」

需求只给了两个方向。按字面只加两条单向规则，会留下一个可达的自相矛盾状态：

```
父任务（进行中）
└── 子任务（待办）     ← 把它从「进行中」拖回「待办」之后，父任务留在进行中
```

所以有未归档子任务的父任务，列**完全推导**：有子任务在进行中 → 进行中；否则子任务全完成 → 完成；
否则 → 待办。这是子任务列的纯函数，与操作历史无关，上面那种状态因此不可达。没有未归档子任务的
任务是**叶子**，保留自己的列，也只有它能被手动拖动。

代价说清：有子任务的父任务在列视图里拖不动，它的列内位置也由对账决定（落到目标列末尾）。为了让
「拖不动」是看得见的而不是拖完弹一句错误，前端 `domain/board.ts` 的 `canDragCard` 与后端两条 `400`
用同一条判据。

### 决策二：父任务不走表

在推导下「进行中」等于「子树里有活在干」。让父任务继续按 `shouldRun` 走表，项目根会把整棵子树的
干活时间都记在自己头上——一个估 5 天的项目会显示「超期 7 天」，而那个数字与它自己的工期估算没有
任何对照关系。所以计时判据加了一条：**只有叶子走表**。

不变式从「`running_since` 非空 ⟺ 进行中且未归档」变成「⟺ 进行中、未归档、且没有未归档子任务」。
两个边界都要处理：任务长出第一个子任务时结算停表；删掉或归档最后一个子任务时，若它仍在「进行中」
就重新开始计时。两条都由对账兜住。

### 决策三：对账放仓储层，且刻意不做成迁移

每个会改到父子关系、列或归档状态的写入口，都在自己的 immediate 事务里、写完之后调一次
`reconcileDerivedStatus`：全表扫未归档任务 → 推导列 → 只写变了的那几行（列变了按目标列的
`MAX(orders) + 1000` 重新取号，计时按新判据结算）。

为什么全表重算而不是逐个入口推算「这次影响了哪些祖先」：个人规模下成本可以忽略，而那种推演只要
漏一处，看板上就会出现一个与子任务矛盾的父任务——最常漏的正是归档（把一个子任务收起来，父任务
从「有子任务」变回叶子，它的表要重新跑起来）。状态本来就一致时它一个字节都不写（`updated_at`
也不动），所以幂等，重复调用安全。
...
- `apps/api/src/domain/derive.ts`（新）：`deriveColumns` 纯函数。迭代式后序遍历（子节点先算完再算
  父节点），父子成环的手工脏数据靠「已在栈上就不重复入栈」终止，不递归所以不会栈溢出；一个真正的
  根都没有的整支环也会被补一轮起点扫到。
- `apps/api/src/domain/columns.ts`：补 `TODO_COLUMN_ID`——推导需要一个明确的「待办」落点，之前只有
  `doing` / `done` 两个常量（待办是隐含的默认值）。
- `apps/api/src/domain/clock.ts`：`shouldRun` 加 `isLeaf` 参数与理由注释。
- `apps/api/src/repositories/tasks.ts`：新增 `reconcileDerivedStatus`、`readMaxOrders`、`ordersKey`、
  `countActiveChildren`；`settleSubtreeClocks` 换成只管归档行的 `stopSubtreeClocks`；`moveTask`、
...
- 前端：`domain/board.ts` 的 `canDragCard`（唯一定义，光标与拖拽准入共用）、`useCardDrag.canBegin`
  ...
  `countChildren().total`，与看板 `childTotal` 同口径）。

### 验证

- `pnpm lint` / `pnpm typecheck` / `pnpm build` / `pnpm test` 全绿：api 303 个用例（新 28 个：`derive.test.ts`
  12 个纯函数用例 + `derivedStatus.test.ts` 15 个接口级用例 + `clock.test.ts` 补 1 个），web 485 个
  （基线 477，新 8 个：`canDragCard` 3、拖拽准入 1、卡片光标与工期胶囊 3、树节点 1）。
- 被测行为的关键几条：子任务进入进行中时整条祖先链跟着走、拖回来一起回待办；兄弟还在干就压过
  「全部完成」；给正在计时的任务加子任务会结算停表、删掉/归档最后一个子任务会重新计时；对账在状态
  一致时不动 `updated_at`。
- 另跑了一遍**进程级**验收（临时脚本，跑完已删）：起 `bin/mailuo.mjs` 配一个临时库，用真实 HTTP
  走完整条链路，27 项断言全过——覆盖三个推导方向、两条 400、父任务不计时、归档/取消归档/删除后
  的叶子状态翻转。写这条脚本时踩到一个自己造的错误假设：删掉唯一的子任务之后那个任务已经变回叶子，
  于是它**可以**手动换列——脚本原本期望 400，是实现对、脚本错。这条正好说明「叶子」是动态的，
  任何按任务 id 记住「这是个父任务」的写法都会过期。
- 启动对账单独验了一遍（第二个临时脚本）：直接按旧规则往库里塞「父任务停在完成但子任务在干」
  与「父任务在进行中且 running_since 非空」两种行，再起服务，6 项断言全过——列被纠正、表被停。
  这条覆盖的是「只读不写的老库也要自洽」，接口级用例走不到（它们每次都经过写入口的对账）。
- 本机跑 `pnpm test` 时要 `env -u NODE_USE_ENV_PROXY`：这个环境变量会让 Node 往子进程 stderr 打一行
  undici 实验性警告，而 `bin/mailuo.test.mjs` 有一条进程级用例断言 stderr 为空。这是环境噪音，与本次
  改动无关，CI 上不存在。
- 既有用例的夹具/断言按新口径改了（`move.test.ts`、`tasks.test.ts`、`archive.test.ts`、`taskClock.test.ts`、
  `TaskCard.test.tsx`、`reminderUi.test.tsx`、`App.test.tsx`）：它们原本依赖「父任务可以停在任意列」
  或「父任务也走表」，新规则下那些状态不再可达。改的时候守的是各自原本要守的东西（其他层不动、
  进度计数、归档位置、工期落库、计时不变式），不是放宽断言——`taskClock.test.ts` 的
  `clockViolations` 反而多了一条「是叶子」的条件，比原来更严。

### 审阅（本步）

按项目规则让子代理审了正确性、安全性、质量。第一个 agent 跑了很久没返回，中断后换了一个范围收窄的
（只读 5 个文件、不跑测试）。它给出 3 条有价值的意见：

1. **真问题，已修**：成环的兜底值会泄漏到邻居。原实现在遇到回边时让「已在栈上」的那个节点退回当前列，
   但它的**邻居**结算时会取到这个兜底值。审阅给的最小反例（甲 parent=乙 column=done、乙 parent=甲
   column=todo、丙 parent=甲）下，乙只有一个子任务甲，于是被算成「完成」，而甲又按乙的兜底值算成
   「待办」——自相矛盾、且换个输入顺序结果就变。更要紧的是它不是不动点：一个成环的库会在**每次写
   请求**里都被改写一遍，这直接推翻了本步到处在说的「状态一致时不写任何行」。改法是把环上那一段
   整个锁住、保留各自的列（不是迭代求不动点：两节点互相当父时代回规则会来回翻，不动点不保证可达）。
   补了两条用例：环上保留各自的列、结果与输入顺序无关；外加一条在真实库里造成环、断言第二次对账
   一个字节都不写。
2. **测试脆弱，已改**：`orders` 用例原本断言「甲拿 1000、乙拿 2000」，而同一批被推导的任务谁先谁后
   由扫描顺序决定（那条 SELECT 没有 `ORDER BY`），等于把实现巧合钉成契约。改成断言「都在这一列、
   号互不相同且是 1000/2000/3000 这一组」。
3. **驳回**：它说「归档路径把子树停表这件事没有任何测试，删掉 `stopSubtreeClocks` 也全绿」。不成立
   ——`taskClock.test.ts` 的「归档一棵子树会把子树里正在计时的任务都停下来」正是断言这个：归档整棵
   子树后逐行查 `running_since` 为 null，并用 `clockViolations` 全库校验不变式。审阅只读了本步新增
   的两个测试文件，没读到那一份。
4. 它提出的两个不确定项都已确认无需改动：`ORDERS_STEP` 是 1000（正数），`updateTaskSchema` 用
   `.refine` 强制 `columnId` 与 `position` 成对出现，所以 `applyTaskUpdate` 里那个成对条件不会
   静默吞掉一次换列。

### 没做的（可选复杂性）

- **子树聚合的时间展示**：父任务卡片现在既不画工期胶囊也不画提醒条。把一个分支的投入表达成
  「Σ 未归档叶子的已用 / Σ 未归档叶子的工期」是下一步，本步只让假数据消失，不引入新口径。
- **抽屉里的工期输入**：有子任务的父任务仍能在详情抽屉里填工期，但那两个数字在界面上没有出口。
  要么下一步做聚合展示时让它当分母，要么那时再把这一栏隐藏/禁用。
- **父任务在推导列里的位置**：只拒绝手动移动，所以它落在目标列末尾，顺序由对账的扫描顺序决定。
  要让它可排序（同列内允许拖、跨列不允许）就得再改落点判定，收益不明确。
- **前端对「为什么拖不动」的解释**：目前只有光标变化，没有 tooltip 或提示文案。

### 合并前复检（终态、只读审核）

上面那次审阅发生在修环的**之前**，所以合并前又让另一个子代理只读审了终态（HEAD `a6ef9cc`）。
结论可合并，未发现正确性 / 幂等性 / 不变式缺陷。它做的验证比接口级用例更狠，值得记下来：

- `deriveColumns`：穷举 n ≤ 6 的全部父子图 × 4 组随机列（504,500 个输入）与一份独立参考实现比对，
  零处不一致；打乱输入顺序结果不变；结果代回是严格不动点。
- `reconcileDerivedStatus`：300 轮 × 25 次随机合法写操作（建 / 移叶子 / 改父 / 归档 / 取消归档 / 删除）
  之后换一个 `now` 再对账，整表快照（含 `updated_at`）零变化——这正是「状态一致时不写行」要守的东西。
- 六条写入口逐条追踪 `running_since ⟺ 进行中 ∧ 未归档 ∧ 叶子`，全库零违反。
- SQL 全参数绑定，`IN (...)` 占位符按数组长度生成，错误响应只回「服务器内部错误」。

它提了 4 条低危、本步不做的：`deriveColumns` 的回边锁定与 `reconcileDerivedStatus` / `moveTask` 还可以再拆
（当前复杂度约 11，eslint 上限 30）；`routes/tasks.ts` 的「列不存在: ${columnId}」会原样回显一个没有长度
上界的 `columnId`（既有模式，是响应放大而非注入）；启动对账不在事务里（中途崩溃下次启动会自愈）。
本步已合入 `main`（`9d0dd6c`）。


## D77 第 39 步：父任务的分支投入用子树聚合表达（2026-09-25，分支 feat/subtree-time，用户授权改规范）

D76 把父任务那条会误导人的进度条摘掉了，但没给它新的出口：父任务卡片上「这个分支投入了多少、
还剩多少」在界面上完全没有答案。本步补上口径，由用户在 3 版 HTML 原型里定版（B 版）。

### 问题

父任务自己的 `duration_minutes` / `spent_minutes` 回答不了这个问题——它的列由子任务推导、
表也是停的（D76），所以任何「拿它自己那两个数画点东西」的方案都是在复刻被摘掉的那条假条。

### 决策一：口径是 Σ 未归档叶子，未估即未估（用户从三版原型里选的 B 版）

- 已用 = Σ 子树里未归档叶子的已用（叶子的已用仍是 `spent_minutes` + 当前这一段）。
- 工期 = Σ 子树里未归档叶子的工期；**只要有一片未归档叶子没估工期，整个分母就算「未估」**。
- 未估时卡片写「已用 X / 未估」（虚线胶囊，与叶子未估工期同一种标记），既不画贴底进度条，也不给
  临近 / 超期；树行只标一个「未估」。全部叶子都估了才回到完整形态（分数 + 条 + 小字）。

为什么不做「未估按 0 计入分母、另挂一个『未估 N』标记」的 A 版：那样分子随叶子干活往上走而分母不动，
比率自己往上飘。用原型里的数据：三片叶子 Σ工期 1 天 4 小时，其中未估的那片正在跑，它的已用全部进
分子、工期记 0，整条显示 92%。这与 D76 摘掉的那条假条是同一类误导——一条来自「自己的表停了」，
一条来自「分母少了」。也没有选 C 版（只报已用、不做分支进度）：那等于取消「给这个分支一个出口」
这个目标，用户仍然答不出「还要多久」。

两个和都是**工作量**口径：叶子可以并行，Σ工期 不是日历工期。这句话写进胶囊的 title 与条的可访问
名字，避免被当成剩余时间读。

### 决策二：汇总在前端算、只有一份实现；树的数据因此提到 BoardPage

`domain/subtreeTime.ts` 是唯一的实现，卡片与任务树共用。它是纯函数，输入是 `GET /api/tree` 那份
平铺任务列表，输出 `Map<任务 id, SubtreeTime>`：

- `leafCount` / `spentMinutes`（Σ 已结算）/ `runningSince[]`（哪几片叶子在跑）/ `durationMinutes`
  （Σ 工期；有未估即 null）。
- 「正在跑的那一段」故意不算死在里面：汇总只记下开始时刻，分钟数由视图用自己的 `now` 现算
  （`subtreeUsedMinutes`），所以父任务的条会和叶子的条一起随时间走，而不是等下一次取数才跳一下。
  取整沿用叶子的那一份实现（`elapsedSinceMinutes`），否则会出现父亲比孩子多一分钟这种对不上的数。

配套改动：任务树的读取从 Sidebar 提到 BoardPage。它本来就是「页面数据的唯一持有者」，而看板卡片也要
按同一棵树算汇总——继续让 Sidebar 自己取，两处就会拿到两份可能不同步的数据。`refreshToken` 那套间接
刷新随之删掉，`refreshAll` 直接调 `tree.refresh()`。

后序遍历用显式栈（树有多深由用户决定），成环的脏数据（只有手工改库才可能）先把环上那几个任务整批
剔出去再汇总：不剔的话结果取决于从哪个节点开始遍历（甲↔乙时，先被结算的那个会把自己的汇总让给
另一个人），同一份数据两次渲染能给出不同的数。理由与 D76 对成环的处理一致。

### 决策三：树行只放得下比例，完整文案进 title

任务树一行是 252px，还要放三角、标题、归档标记与「2/3」徽标，塞不下「已用 1 天 3 小时 / 1 天 4 小时」。
所以父任务行只画迷你条 + 百分比（超期用危险色），未估时只标「未估」，完整文案在条与标记的 `title` 上。
代价是触屏与键盘拿不到 title，这是这道题上能接受的那一半损失。

### 实现清单

- 新增 `apps/web/src/domain/subtreeTime.ts`：`buildSubtreeTimes` / `subtreeUsedMinutes` / `branchView`。
- `domain/reminder.ts`：判定抽成 `reminderFromTotals({duration, used, running, done, archived})`，
  `reminderView` 变成它的薄包装，另导出 `elapsedSinceMinutes`。父任务的聚合提醒与叶子因此共用同一次
  判定，不会出现「叶子算一档、父任务算另一档」。
- `TaskCardFace`：父任务多一行聚合胶囊（单独占一行，260px 的列放不进 meta 行）；`TaskCard` /
  `Column` / `BoardView` 逐层透传 `subtreeTimes`（与既有 `nowMs` 同一种透传方式）。
- `TreeNodeRow`：父任务行画聚合条 + 比例或「未估」；顺手把右侧状态拆成 `RowStatus` 与 `RowBadge`
  ——这处拆分把它的圈复杂度从 36 压回 eslint 的上限 30 以内。
- `Sidebar`：改吃上层给的 `tree` 与 `subtreeTimes`，不再自己取树。
- `App.tsx`：`useTree` 提到 `BoardPage`，用 `useMemo` 算出汇总表交给 Sidebar 与 BoardView。
- `docs/spec.md`（用户验收后授权改规范）：「状态语义」里父任务那条改成子树聚合的口径，
  「工期提醒」里「有子任务的父任务不提醒」改成「按子树聚合判定」，界面行为的列表补上卡片与树行
  各画什么。

### 验证

- `pnpm lint` / `typecheck` / `build` / `test` 全绿：web 500 个用例（基线 485，新 15 个：
  `subtreeTime.test.ts` 11 个纯函数用例、`reminderUi.test.tsx` 3 个界面用例、`App.test.tsx` 1 个接线
  用例）。api 未改动，303 个用例照旧。lint 仍是 5 条既有 warning、0 error。
- 界面上改口径的两条旧用例按新行为重写而不是放宽：卡片那条从「父任务不画任何条」改成「画 Σ 的条，
  未估时不画」；树那条从「父节点不画条」改成「画聚合条与比例」。另补一条「汇总拿不到时父卡片宁可
  不画，也不拿自己的工期顶上」——这条守的正是 D76 摘掉的那类假数据。
- 真机验收（临时数据与脚本，跑完已删）：塞一棵含「有未估叶子的父任务 / 全估且超期的父任务 /
  单子已完成的父任务 / 普通叶子 / 归档整支」的树，用真实 `tsx` 起的 API + 真实 SQLite + 构建产物在
  Chrome 里看了浅色下的根看板与内层看板：三种父任务卡片与树行的形态都与定版原型一致，胶囊没有溢出，
  叶子卡片与旧行为逐字一致。
- 本机环境两条注意（与本次改动无关）：跑 `pnpm test` 要 `env -u NODE_USE_ENV_PROXY`（`bin/` 那条
  进程级用例断言 stderr 为空）；这一步的 worktree 里 `node_modules` 是从主检出拷过来的（pnpm 在线装
  依赖在本机多次超时），跑脚本要带 `--config.verify-deps-before-run=false`，否则 pnpm 会先试着补装。

### 没做的（可选复杂性）

- **抽屉里的工期输入**：有子任务的父任务仍能在详情抽屉里填工期，但那个值不参与子树聚合（聚合只看
  叶子），界面上也没有出口。要么下一步把这一栏对有子任务的任务隐藏 / 禁用，要么把它定义成
  「覆盖聚合的分母」——两种都行，但得先定一个，现在的状态是「能填、没人看」。
- **树上「未估」标记的数量**：B 版只说「有叶子未估」，不说有几片。`SubtreeTime` 里没有这个数，
  要显示得再加一个字段（C 版原型那种悬停明细同理）。
- **父任务在推导列里的位置**：与 D76 相同，仍由对账的扫描顺序决定，不可排序。

### 审阅（本步）

按项目规则让子代理只读审了正确性、安全性、质量，结论可合并。它给了 5 条意见，只有第 1 条成立：

1. **成立，已改**：`App.tsx` 的 BoardPage 类注释还写着「任务树（数据在 Sidebar 内部取，靠
   refreshToken 触发）」，而这正是本步改掉的东西。已按现状重写。
2. **驳回**：它说「父卡片 meta 行照画自己的工期胶囊」（`TaskCardFace.tsx:89`）。不成立——那一整块在
   `!hasChildren &&` 里面，只对叶子生效；真机验收的父卡片上也没有第二个胶囊。它还说「树行拿不到汇总时
   会退回画父任务自己的条」，同样不成立：`TreeNodeRow` 是
   `branch !== null ? branch.reminder : total === 0 ? reminderView(...) : NO_REMINDER`，
   `total > 0` 又拿不到汇总时就是 NO_REMINDER，与卡片同一条判据。
3. **驳回**：它给的最小反例（甲↔乙成环、丙挂在甲下）说「丙在一个视角消失、在另一个视角被算」。不成立：
   环上那两个整批不进遍历，压根没有汇总条目，丙只有自己那条全零的条目（`subtreeTime.test.ts` 的成环
   用例钉的就是这个）。它顺带说 `sumLeaves` 里 `part === undefined` 的注释「正常走不到」与实际相反，
   也不成立：剔环之后那条守卫确实不可达，是留给类型收窄的。只把「挂在环下的子树不再向上合并」这句
   补进了注释。
4. **驳回**：它算出树行会同时出现「迷你条 + 百分比 + 未估 + 归档 + 徽标」（约 143-165px）把标题挤没。
   这几项互斥：「未估」只在分母不可信时出现，而那时按口径根本不画条与百分比；归档的父任务整支都已
   归档，`leafCount` 是 0，也不会出现「未估」。实际最宽的组合是「条 + 百分比 + 2/3」，真机验收里标题
   仍然可读。
5. **驳回**：它说本步把 lint warning 从 5 条变成 6 条（`Sidebar.tsx` 的 `tasks` 那条是本步新增）。
   不是：那条 warning 在本步之前就在（只是行号从 73 挪到 81），改动前后都是 5 条 warning、0 error。

顺手把「汇总拿不到就什么都不画」那条用例的断言加严了一格（同时断言不出现「已用」）——这正是审阅
第 2 条想守的行为。
...
D77 之后还剩一处打架：抽屉里的「工期」对有子任务的父任务同时是两种意思——它是父任务自己的估算
（依赖图排期读它），而卡片上画的是子树叶子的汇总。同一个任务两个数，用户没法判断哪个是哪个。
补一段说明文字是最差的路（用户的原话：需要额外的文字说明就不够优雅）。

### 决策一：工期是叶子的属性，父任务那一栏只读（用户定版抽屉原型 B2）

- **叶子**：抽屉里照旧是三个输入框 + 「未估工期 / 瞬时」两个快捷按钮，一个字不改。
- **父任务**：只显示一行只读值 + 一个 11px 的「按子任务汇总」标记，没有输入框外形。值 = Σ 未归档
  叶子工期；有叶子未估显示「未估」；汇总还没取到（任务树没读回来）显示「—」，不拿它自己那个值顶上。
  判据用 `childTotal`（未归档直接子任务数），与看板卡片、后端 `countActiveChildren` 同一个口径，
  所以「最后一片子任务被删掉/归档之后又能改」自动成立，不用另记状态。
- **后端也拦**：`PATCH /api/tasks/:id` 带 `durationMinutes` 且该任务有未归档子任务 → 400
  「任务有子任务，工期由子任务的工期汇总，不能单独设置」；写 `null`（想清回未估）同样拒绝，
  免得借这一步绕过。与 D76「前端不给拖 + 后端两条 400」同一个形状：前端不做无用功，后端是权威。
- 库里已有的父任务工期**不清**：字段保留、口径作废。与 D76/D77 对字段的处理一致——删数据是不可逆的，
  而它已经不参与任何计算与显示。

### 决策二：依赖图里父节点也改用同一份汇总（用户选的「一起改」）

不改的话是最难看的形态：抽屉与卡片写着「1 天 4 小时」，依赖图节点上却挂着一个冻结的旧值或者「未估」。
所以层排期（`repositories/deps.ts` 的 `readLayerSchedule`）对「有未归档子任务」的节点改用子树汇总：

- 新实现 `apps/api/src/domain/subtreeDuration.ts`：纯函数，与前端 `domain/subtreeTime.ts` 同一口径。
  跨包没有共享代码的位置，重复这一份是取舍（与 `lib/tree.ts` 的 `countChildren` 重复后端
  `child_total` 同一类），两边的用例刻意用同一组夹具钉住同样的行为。
- 取值规则：有未归档子任务的任务用汇总（有叶子未估即 `null` → 界面「未估」、CPM 按 0 参与）；
  其余（叶子、已归档任务）用自己那个 `duration_minutes`。已归档整支不进汇总。
- 用一条全库 SELECT 换掉递归 SQL：父任务的叶子在更深的层里，这一层的查询看不到它们。个人规模下
  这点开销可以忽略（与 `reconcileDerivedStatus` 每次写都全表重算同一个取舍）。

代价要记清楚：Σ 叶子是**工作量**口径，分支里的任务可以并行，拿它当这一层的日历工期会高估；
「给一个分支单独估个日历工期」这个能力从此没有了。这是用户明确选的（原话：「依赖图一起改」）。
真正理论上更对的做法是让 CPM 递归地用子树自己的关键路径长度，那是独立的一个功能，本步不做。

### 实现清单

- 新增 `apps/api/src/domain/subtreeDuration.ts`：`subtreeLeafDurations`。
- `apps/api/src/repositories/tasks.ts`：新增 `listActiveDurationNodes`（全库未归档任务的 id/父子/工期），
  三个读入口共用（层排期、搜索）。
- `apps/api/src/repositories/deps.ts`：层排期改用汇总后的工期（算时间与响应里那个数用同一个值）。
- `apps/api/src/repositories/search.ts`：搜索结果行的工期也换成同一份汇总。**这一处是审阅抓出来的**：
  不带这一手，搜索页会继续画父任务那个已经退休的 `duration_minutes`——父任务库里存 999、
  叶子汇总 720 时，同一张任务在卡片/抽屉/依赖图上写 720、在搜索结果行上写 999。
- `apps/api/src/routes/tasks.ts`：`PATCH /api/tasks/:id` 新增 `DERIVED_DURATION_REJECTED` 400，
  排在「有子任务不可手动换列」之后；`hasChildren` 只查一次，两条拒绝共用。
- `apps/web/src/components/TaskEditorPanel.tsx`：新增 `subtree` prop；父任务渲染只读行（B2），
  保存时**不发** `durationMinutes`；`canSave` 对父任务不再看工期草稿的合法性。
- `apps/web/src/App.tsx`：把 `subtreeTimes.get(editing.id)` 交给抽屉。
- `docs/spec.md`（用户验收后授权改规范）：状态语义那条补上「工期是叶子的属性、父任务只读且 400」，
  关键路径补一条节点工期的口径，界面行为那条补上「工期只有叶子能改」。

### 验证

- `pnpm lint` / `typecheck` / `build` / `test` 全绿：api 320 个用例（基线 303，新 17：
  `subtreeDuration.test.ts` 6 个纯函数、`cpm.test.ts` 5 个接口级、`derivedStatus.test.ts` 4 个 400/放行、
  `search.test.ts` 2 个汇总口径），web 501 个（基线 500，新 1 个父任务抽屉用例；另两条既有抽屉用例
  改用叶子任务，因为「支付对账」现在是父任务、工期只读）。
- 写这份实现时踩到一个自己造的错：`subtreeLeafDurations` 一开始给**每片叶子**也留了条目（值是 0），
  调用方用「表里有没有这个 id」判断是不是父任务，于是所有叶子的工期都变成 0、CPM 项目工期全归零
  （8 条既有用例当场变红）。改成只给有子任务的任务写条目。纯函数用例里那条「没有任何未归档子任务
  的任务不在表里」正是钉这个的。
- 真机验收（真实 API + 真实 SQLite + 浏览器，数据是临时库，看完即删）：父任务「数据迁移」库里存着
  3000 分钟、两片叶子 480 + 240，抽屉显示「工期 1 天 4 小时 · 按子任务汇总」，`/api/board/cpm` 里
  这个节点也是 720，同一层另一个未估的父任务两边都是「未估」；叶子抽屉与改动前逐字一致。

### 审阅（本步）

按项目规则让子代理只读审了正确性、安全性、质量。结论可合并，3 条发现，前两条与第三条的第一半成立：

1. **成立，已修**：搜索结果行画的是父任务那个已退休的 `duration_minutes`（`search.ts` 的
   `toSearchResult` 直接用了 `row.duration_minutes`，而 `SearchResult` 里没有 childTotal）。
   已改成同一份汇总，并补了「全估给汇总值 / 有叶子未估给 null」两条用例。
2. **成立但决定不改**：已归档的父任务若还有未归档子任务（**只可能来自手工改库**：归档会连整棵子树
   一起归档），层排期会给它自己那个值，而卡片与抽屉什么都不显示（前端把归档任务整支排除在汇总外）。
   要「一致」就得让 CPM 也给归档节点算汇总或者干脆给未估，两种都是为脏数据加分支。已把这条记进
   「没做的」，选择保留现状：`derived` 里没有它 → 用自己那个 `duration_minutes`，一条规则走到底。
3. **成立，已修**：只读行的 `title` 恒定写「任务树还没取到时暂时算不出汇总」，而 `—` 也可能来自
   脏数据。改成只在 `subtree === null` 时给这句，其余情况不给 title。
4. **驳回（并给出更强的结论）**：它建议补一条「父任务挂着一个环」的用例，说那是 `sumLeaves` 里
   `part === undefined` 唯一可能生效的形状。这个形状**根本不存在**：每个任务只有一个 `parentId`，
   环上的人不可能同时属于环外，所以环一定是封闭的、不挂在任何非环任务下面。我把那条守卫的注释改成
   了这一点，并补了一条真正有用的用例：环存在时非环的父子链照常汇总、环上两个都不进表。

### 没做的（可选复杂性）

- **父任务自己的旧值**：仍留在库里，不显示也不参与任何计算；要彻底清掉得手工改库。没有做一次性清理，
  因为删数据不可逆，而它已经不产生任何影响。
- **已归档但有未归档子任务的父任务**：这种行只可能来自手工改库（归档会连整棵子树一起归档）。
  它不在汇总表里，所以三个读入口（层排期、搜索）都用它自己那个 `duration_minutes`，而卡片与抽屉
  什么都不显示。没有为它加分支：一条规则（「表里没有 → 用自己那个值」）比两种特例更容易讲清楚。
- **依赖图节点上的口径标记**：图上父节点显示的就是汇总值，但没有「按子任务汇总」这类标记。用户明确
  嫌额外文字说明，而图上的数与卡片、抽屉是同一个数，所以不加。
- **递归 CPM**：让分支的日历工期等于它自己子树的关键路径长度（并行感知），父任务就连汇总都不需要；
  那是独立的一个功能。


## D79 第 41 步：发 0.3.0（2026-09-25，分支 chore/release-0.3.0）

用户要求把 0.2.0 之后进包的改动发出去，并确认版本号走 **0.3.0**。

### 这一版进包的改动（相对 0.2.0 的发布源 `ef315a9`）

三步都在 2026-09-25 合入，都改到了发布产物（`apps/api/dist` 与 `apps/web/dist`）：

- **父任务的状态由子任务推导**（D76）：子任务里有一片在进行中，这条分支就算进行中；子任务全部完成
  才算完成，其余算待办。配套的是父任务不再计时——它的列是推出来的，`running_since` 对父任务恒为
  NULL，卡片上也不再出现计时与进度条。**这是一条行为变化**：老库里手工拖到「进行中」的父任务会在
  启动对账时被拉回推导列，父任务上正在跑的钟会被停掉（叶子不受影响）。
- **父任务的分支投入按子树聚合**（D77）：卡片多一行聚合胶囊、任务树多一条迷你条。口径是
  Σ 未归档叶子的已用 / Σ 未归档叶子的工期，只要有一片未归档叶子没估工期，整条就标「未估」而不是
  按 0 计入分母。这是新的展示，也是 D76 摘掉的那条假条（拿父任务自己的数字画）的替代品。
- **工期是叶子的属性**（D78）：抽屉里父任务的工期变成只读的一行（汇总值 + 「按子任务汇总」），
  接口上 `PATCH /api/tasks/:id` 对**有未归档子任务的任务**带 `durationMinutes` 一律 400（判据是
  `countActiveChildren > 0`，所以子任务全归档之后它又是叶子，可以正常写）。关键路径、
  搜索结果、卡片、抽屉四处现在读同一个汇总值，不会再出现同一张任务两个工期。
- 不进包但已合入的：D76–D78 三处经用户授权改的 `docs/spec.md`，以及三步的决策记录。

升级不需要动数据库：`apps/api/migrations/` 与 0.2.0 逐字相同（仍是 001–003），推导与汇总都在代码里。
父任务留在 `duration_minutes` 里的旧值不再显示、也不参与任何计算；它仍在 API 的响应里（没有为删列
加第 4 个迁移），前端也照样读进来，但对有子任务的任务那一栏走的是汇总值。本步不做一次性清理
（见 D78 的「没做的」）。

### 为什么是 0.3.0

D75 为 0.2.0 说过同类理由（行为变化所以不走补丁号），但这条通则没有在仓库里写成条文，本步沿用
同一个判断：上面第一条改的是「父任务点开是什么列、钟在不在跑」，升级后直接看得见；第三条收紧了
一个既有接口的入参。所以不是 0.2.1。

### 做法

与 D75 同一条链路，走 worktree：

1. `.worktrees/chore-release-0.3.0` / 分支 `chore/release-0.3.0`，三个 `package.json`（根、
   `apps/api`、`apps/web`）一起改成 0.3.0（只有根那个会被发布，理由同 D75）。
2. 合并回 main，推送。
3. 建 Release：tag `v0.3.0`，target 是 main 顶端（也就是合并后的那个提交，含 `release.yml`），
   非 draft、非 prerelease → `guard` → 门禁 → `npm publish`。

一处此前没注意到的状态：`origin/main` 还停在 `ef315a9`（`v0.2.0` 的那个合并提交），D76–D78 的
10 个提交只在本地。所以这次推送顺带把三天的改动第一次送上 GitHub，远端 CI 也是第一次跑这三步的
终态（tag 上那次发布门禁另算，它跑的是同一串命令）。

### 顺带补的两处文档

- **索引缺了 D76–D78**：D73 定的规矩是「新增条目时在对应段末补一行」，D76–D78 三步都没补，
  索引在 D75 之后就断了。本步补上 2026-09-25 段的四行。
- `docs/development.md` 的版本示例从 `v0.2.0` 换成 `v0.3.0`：D75 没换（留着旧版本号当示例不算错），
  但发版这一步是它最容易过期的时候。
...
- 不写 CHANGELOG 文件：同 D75，改动清单就在上面，仓库还没有「每版一份 changelog」的约定。
