...
**让位动画**：卡片位置变了就用 FLIP（先按位移前的位置摆好，下一帧放开 `transform 120ms` 过渡），**只在拖拽期间**开。平时新建、归档、换看板也会改卡片位置，那些场合加动画只会显得飘（D35 的静默重取会重画整块看板，加了动画就是一片乱动）。三个实现要点写在 `BoardView.useBoardCardFlip` 里：量尺寸前先清掉自己上一轮留下的 transform、`transition` 留到下一轮渲染再清、**`board` 必须在依赖数组里**（少了它，`enabled` 一个拖拽里只从 false 变到 true 一次，effect 不会再跑，动画永不执行——审阅发现的）。
