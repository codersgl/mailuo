...
**收起时把焦点交给收起按钮。** 追踪：键盘用户在树里选中过某个节点（焦点在树里），点收起后树变成 `display:none`，浏览器把焦点退回 `body`；下一次 Tab 从页面开头重新开始，用户会以为「界面把我扔回去了」。所以在 `collapsed` 变 true 的那次 effect 里 `toggleRef.current?.focus()`。只写 effect、不在点击处理函数里抢焦点：这样首次挂载（本地存的本来就是收起）不会抢用户焦点，只有真正发生「展开 → 收起」时才动焦点。按钮本身带 `focus-visible` 环，且用的是全站同一串类（`TaskCard` 的「⋯」、`Column` 的「+」、`SearchBox` 的清空、`ThemeToggle` 都是这一串），收起后接住焦点时能看见环。
...
**偏好仍然只存前端**：`kanban.tree.panelCollapsed` 存单个 boolean（`TREE_COLLAPSED_KEY`），坏值（不是 boolean）按展开算，与 `showArchived` 同一套兜底。`docs/spec.md` 已经写了「展开折叠状态只存在前端本地存储，不落库」，所以这一步后端一行没动，接口与数据模型都不用改。
...
**验证方式**：单测 438 项（api 143 / web 295，其中新增 4 项守着收起行为）+ 两端类型检查 + 生产构建；另在真实浏览器里跑 18 项验收（构建产物 + 真实 API + 用户库副本，脚本不入库、放在 `.tmp-verify/`，验收完删）：宽度 252/44、收起后树不可见但没卸载、看板多出 208px、开关不渲染、按钮名字与 `aria-expanded`、焦点落点、键盘路径的聚焦环、偏好落盘、刷新后保持、深色下的配色、顶栏不溢出、回车展开、展开不重新取数、无未捕获异常。
