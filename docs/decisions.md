...
- 后半段换算不能省：`position` 的口径是**未归档兄弟里的插入下标**，而列里可能混着不参与重排的归档卡片（D20），于是「渲染顺序里的下标」和它不一样。`positionForDrop` 照渲染顺序数未归档卡片，锚点本身也可能是归档卡片（拖到它上方时），这时退回列尾是错的，所以不能只在未归档子集里找锚点。
...
**让位动画**：卡片位置变了就用 FLIP（先按位移前的位置摆好，下一帧放开 `transform 120ms` 过渡），**只在拖拽期间**开。平时新建、归档、换看板也会改卡片位置，那些场合加动画只会显得飘（D35 的静默重取会重画整块看板，加了动画就是一片乱动）。两个顺序问题写在 `BoardView.useBoardCardFlip` 的注释里：量尺寸前先清掉自己上一轮留下的 transform，`transition` 留到下一轮渲染再清。
...
**文件树拖动**：与卡片共用同一套指针语义，但不克隆卡片（树里那一行本来就在，克隆反而看不清落点），只是被拖的行降透明度、候补父级整行高亮 + 2px 强调色左竖条。落点分两段：悬停行上半区 = 成为它的子节点，下半区 = 排到它后面成为兄弟；目标本身是顶层节点时下半区也只能挂到根下（顶层没有兄弟可插）。拖到自己或自己的后代下不显示任何落点、松手也不生效——后端另有 400 环检测兜底。落库时把任务**当前所在列**带过去，后端只会把它追加到那一列末尾，所以这一步只换层级、不换列。成功后只重取树：改父级不影响当前看板的卡片与面包屑。

**点击与拖拽的分界**：阈值 4px，之内的位移仍是一次点击（进入子看板）。超过阈值后浏览器仍会在 `pointerup` 后补一个 `click`，两个 hook 都用「微任务清标记」吞掉这一次点击（`pointerup` 与 `click` 在同一条任务链上，`queueMicrotask` 一定晚于 click）。不拦的话拖完一张卡片会顺手进入它的看板。

**可选复杂性，这次没做**：贴近视口边缘的自动滚动、键盘可达的拖拽、拖动时自动展开折叠的父节点、克隆卡片跟随卡片高度变化（拖拽期间卡片不会变高）。个人使用下先接受。

**验收方式**：jsdom 测不了指针拖拽（没有布局、`elementFromPoint` 未实现），所以用 headless Chrome + CDP 派发真实鼠标事件跑了一遍：跨列插到锚点前、同列排序、列尾空白落点、列外松手撤销、阈值内仍是点击、树改父级与拒绝拖到后代，DOM 与后端落库都对上。这类脚本不入库，验收完即删；入库的是纯逻辑单测（`board.test.ts`、`tree.test.ts` 的 `resolveTreeDrop`/`descendantIds`、`useAsync` 的 `mutate`、`useCardDrag.test.tsx` 的阈值与取消语义）。
