...
3. **审查报告没有状态**：352 行、60 条结论里约一半已在 D55–D71 落地，正文却仍是「建议改代码」的
...
5. **同一事实写两到三遍且没写「以谁为准」**：技术栈与目录结构（`development.md` vs `spec.md`）、
   CLI 参数（README vs `spec.md` vs `--help`）、`KANBAN_DB_PATH` 的两个默认值（CLI 是
   `~/.mailuo/kanban.db`，服务端是 `data/kanban.db`，各自都对但没有一处说明这是两种入口）。
...
- `docs/audit-2026-09-23.md` 加「修复状态（2026-09-24 补记）」：已落地条目按决策编号归拢
  ...
  D1→D61、B2/B4–B7/B9/B10/D2→D62、D7 的门禁部分→D70），仍未处理的 17 项逐条写现状，其中 C5
  标明是开放的产品决定（D54 记为待拍板）。报告头部写明正文的行号与计数停在 `b2722de`。
...
- **索引不引入生成脚本**：71 行索引由脚本一次生成、之后手工维护。彻底的做法是
...
- 不处理审查报告「第 8 步」的加固项：C5 要用户拍板，其余按报告自己的判断「等真遇到问题再动」。
