...
- 个人使用
- 命令行启动：一个进程同时提供接口与页面，命令行参数全部有默认值

## 命令行运行

发布到 npm 后（见 `docs/intend.md` 的待办）用：
...
从源码仓库跑同一套入口是 `node bin/mailuo.mjs`，参数一致。它加载的是构建产物，不读源码，
所以要先构建一次：
...
| 参数 | 环境变量 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `-p, --port` | `PORT` | `3001` | 端口被占用时自动往后试 20 个；显式指定的端口不替换 |
...
**从开发模式换过来注意**：命令行启动的库是 `~/.mailuo/kanban.db`，开发模式（`pnpm dev:api`、
`pnpm start`）仍然是仓库根的 `data/kanban.db`。两边不是同一个文件，需要哪边的数据就用
`--db` 指过去，例如 `node bin/mailuo.mjs --db data/kanban.db`。程序不会自动搬迁。
